...
public class SunLightControl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _gameClock._gameMinute.OnChange((prev, curr) => OnMinuteChange());
    ...
    void OnMinuteChange()
    {
        ...
        // sunset
        if (_gameClock._gameHour.Value >= _sunsetStartGameHour24h && _gameClock._gameHour.Value < _sunsetEndGameHour24h) {
            _lightState.Value = LightStates.Sunset;
            TimeAdjustLightWithGradient(_sunset);
            return;
        }
        // sunrise
        if (_gameClock._gameHour.Value >= _sunriseStartGameHour24h && _gameClock._gameHour.Value < _sunriseEndGameHour24h) {
            _lightState.Value = LightStates.Sunrise;
            TimeAdjustLightWithGradient(_sunrise);
            return;
        }
        // daytime
        if (_gameClock._gameHour.Value >= _sunriseEndGameHour24h && _gameClock._gameHour.Value < _sunsetStartGameHour24h) {
    ...
    void TimeAdjustLightWithGradient(Gradient gradient) {
        ...
        int _hack = _gameClock._gameMinute.Value == 0 ? 1 : 0;

        float _current = (_gameClock._gameHour.Value + _hack) * 60f + _gameClock._gameMinute.Value;
        debug = _current;
        float _input;
